...
| ID | Test Case | Steps | Expected Result | Status | Comment |
|----|-----------|-------|-----------------|--------|---------|
...
### 2.4 Cancel/Unschedule Event (Pass)
...
### 3.2 Schedule via Event Click (Pass)
...
| ID | Test Case | Steps | Expected Result | Status | Comment |
|----|-----------|-------|-----------------|--------|---------|
| PAR-001 | Add participant | 1. Open own event<br>2. Click "Edit"<br>3. Select user from dropdown<br>4. Click "Add" | - Participant added<br>- Success: "Participant added successfully" | Pass | |
| PAR-002 | Add duplicate participant | 1. Add user as participant<br>2. Try to add same user again | - Error: "This user is already a participant" | Pass | |
| PAR-003 | Non-creator adds participant | 1. Open other's event<br>2. Try to add participant | - Add button not visible<br>- View-only mode | Pass | |

### 4.2 Remove Participants

| ID | Test Case | Steps | Expected Result | Status | Comment |
|----|-----------|-------|-----------------|--------|---------|
| PAR-004 | Creator removes participant | 1. Open own event<br>2. Click "Edit"<br>3. Click "Remove" next to participant | - Participant removed<br>- Success: "Participant removed successfully"<br>- Invitation removed from participant's mailbox | Pass | |
| PAR-005 | Participant removes self | 1. Open event as participant<br>2. Try to remove self | - Remove button not visible for self | Pass | |

### 4.3 Response Management (Pass)
...
### 5.2 Mailbox / Invitation System

| ID | Test Case | Steps | Expected Result | Status | Comment |
|----|-----------|-------|-----------------|--------|---------|
| MBX-001 | View pending invitations | 1. Login as User B<br>2. User A invites User B to an event<br>3. Check mailbox in top-right | - Pending invitation appears in mailbox<br>- Shows sender (User A), event title, time<br>- Response buttons: Accept/Decline/Tentative | | |
| MBX-002 | Iterate invitation initially unscheduled | 1. User A schedules event at 10AM<br>2. Add User B as participant<br>3. Login as User B<br>4. Check calendar | - Event NOT shown on calendar<br>- Event only in mailbox<br>- Calendar remains empty for pending invitations | | |
| MBX-003 | Event shows after accept | 1. Open pending invitation<br>2. Click "Accept" | - Event moves to calendar at scheduled time<br>- Status becomes "scheduled"<br>- Mailbox entry removed | | |
| MBX-004 | Accept with higher priority conflict | 1. User B has Priority 1 event at 10AM<br>2. Receive invitation to Priority 3 event at 10AM<br>3. Click "Accept" on invitation | - Acceptance rejected<br>- Error: "Cannot accept: conflicts with higher priority event"<br>- Invitation stays in mailbox | | |
| MBX-005 | Accept with lower priority conflict | 1. User B has Priority 5 event at 10AM<br>2. Receive invitation to Priority 1 event at 10AM<br>3. Click "Accept" on invitation | - Acceptance successful<br>- Priority 5 event unscheduled<br>- Priority 1 event scheduled | | |
| MBX-005b | Accept with equal priority conflict | 1. User B has Priority 3 event at 10AM<br>2. Receive invitation to Priority 3 event at 10AM<br>3. Click "Accept" on invitation | - Acceptance rejected<br>- Error: "Cannot accept: conflicts with equal or higher priority event"<br>- Invitation stays in mailbox | | |
| MBX-006 | Decline invitation | 1. Open pending invitation<br>2. Click "Decline" | - Participant response = 'declined'<br>- Mailbox entry removed | | |
| MBX-007 | Maybe response | 1. Open pending invitation<br>2. Click "Maybe" | - Participant response = 'tentative'<br>- Mailbox entry stays with yellow badge<br>- Can change response later | | |
| MBX-008 | Change from Maybe to accept | 1. Click "Maybe"<br>2. Later click "Accept" | - Participant response = 'accepted'<br>- Event appears on calendar<br>- Mailbox entry removed | | |
| MBX-009 | Change from Maybe to decline | 1. Click "Maybe"<br>2. Later click "Decline" | - Participant response = 'declined'<br>- Mailbox entry removed | | |
